...
import './pure-graph-node.styles.css';
...
export function PureGraphNodeComponent<S extends RelinkGraphSchemes>(props: Props<S>) {
  ...
  return (
    <div ref={ref} className={"base-graph-node pure-graph-node " + classNames({"base-graph-node--selected": props.data.selected})}>
      <div className="header flex flex-col text-white pl-4 pr-4 pt-2 pb-2">
        <div className="flex flex-row items-center space-x-2">
          <SquareFunction size="28" />
          <div className="title">{props.data.nodeType} ({props.data.nodeType})</div>
        </div>
